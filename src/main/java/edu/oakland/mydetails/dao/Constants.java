...
public class Constants {
  ...
  public static final String GET_DEGREES =
      "SELECT\n"
          + "    DISTINCT\n"
          + "    COALESCE(degree_status,lead(degree_status) OVER (partition by primary_degree, primary_major ORDER BY rownum)) degree_status,\n"
          + "    COALESCE(class_standing,lag(class_standing) OVER (partition by primary_degree, primary_major ORDER BY rownum)) class_standing,\n"
          + "    primary_college,\n"
          + "    primary_degree,\n"
          + "    primary_level,\n"
          + "    primary_major,\n"
          ...
          + "        CASE WHEN sgbstdn_term_code_eff > souf_get_curr_fut_termcode(sysdate) THEN stvterm_desc ELSE baninst1.f_student_get_desc('STVCLAS',(f_class_calc_fnc(:pidm, sgbstdn_levl_code, souf_get_curr_fut_termcode(sysdate))), 30) END class_standing,\n"
          ...
          + "    FROM saturn.SGBSTDN sgbstdn_a, saturn.stvterm--, SATURN.STVDEGS stvdegs\n"
          + "    WHERE sgbstdn_a.SGBSTDN_PIDM = :pidm\n"
          + "    AND stvterm_code = sgbstdn_term_code_eff\n"
          + "    AND sgbstdn_a.sgbstdn_stst_code in (select stvstst_code from saturn.stvstst where stvstst_reg_ind = 'Y')\n"
          + "    AND sgbstdn_a.sgbstdn_term_code_eff >= (\n"
          + "        SELECT MAX(sgbstdn_b.sgbstdn_term_code_eff)\n"
          + "        FROM saturn.sgbstdn sgbstdn_b\n"
          + "        WHERE sgbstdn_a.sgbstdn_pidm = sgbstdn_b.sgbstdn_pidm\n"
          ...
          + "    FROM SATURN.SHRDGMR shrdgmr, SATURN.STVDEGS stvdegs\n"
          + "    WHERE shrdgmr.SHRDGMR_DEGS_CODE = stvdegs.STVDEGS_CODE\n"
          ...
          + "        SHRDGMR_ACTIVITY_DATE AS activity_date\n"
          + "    FROM SATURN.SHRDGMR shrdgmr, SATURN.STVDEGS stvdegs\n"
          + "    WHERE stvdegs.STVDEGS_CODE = SHRDGMR_DEGS_CODE\n"
          + "    AND shrdgmr.SHRDGMR_PIDM = :pidm\n"
          + "    AND SHRDGMR.SHRDGMR_COLL_CODE_2 IS NOT NULL\n"
          + "    ORDER BY\n"
          + "    activity_date DESC)".replaceAll("\\s+", " ");
